...
carto::transform::Rigid3d pre_pose;
carto::common::Time pre_time;
...
std::unique_ptr<carto::io::PointsBatch> HandleMessage(
  ...
  // add by galyean, use motion filter to reduce points
  if(point_cloud.points.empty()){
    return nullptr;
  }
  carto::transform::Rigid3d current_pose;
  carto::common::Time current_time;
  size_t last_id = point_cloud.points.size()-1;
  current_time = point_cloud_time + carto::common::FromSeconds(point_cloud.points[last_id][3]);
  if (!transform_interpolation_buffer.Has(current_time)) {
      return nullptr;
  }
  current_pose = transform_interpolation_buffer.Lookup(current_time);
  if (current_time - pre_time <= carto::common::FromSeconds(0.1)&&
    (current_pose.translation() - pre_pose.translation()).norm() <=
          0.01 &&
          carto::transform::GetAngle(current_pose.inverse() * pre_pose) <=
          0.001) {
    return nullptr;
  }
  ...
  for (size_t i = 0; i < point_cloud.points.size(); ++i) {
    ...
    Eigen::Vector3f origin = sensor_to_map * Eigen::Vector3f::Zero();
    // add by galyean, we use normals to store the origin of corresponding point
    points_batch->normals.push_back(origin);
...
void AssetsWriter::Run(const std::string& configuration_directory,
      ...
      //   // read all human points before handle assets_writer pipline
      // {
      //      const carto::mapping::proto::Trajectory& trajectory_proto =
      //                               pose_graph_.trajectory(0);
      ...
      //     std::ifstream load_static_human_points("/media/galyean/DATA/human_points/static.txt");
      //     std::ifstream load_spin_human_points("/media/galyean/DATA/human_points/spin.txt");
      //     char buffer[1024];
      //     while(load_static_human_points.getline(buffer,1024)){
      //       float values[4];
      //       std::stringstream ss;
      //       ss<<buffer;
      ...
      //         ss>>values[i];
      //       }
      //       carto::common::Time point_t = ::cartographer_ros::FromRos(ros::Time(values[0]));
      //       Eigen::Vector3f p_xyz_local(values[1],values[2],values[3]);
      //       if (!transform_interpolation_buffer.Has(point_t)) {
      //           continue;
      //       }
      //       const carto::transform::Rigid3d tracking_to_map =
      //                   transform_interpolation_buffer.Lookup(point_t);
      //       const carto::transform::Rigid3d sensor_to_tracking =
      //                   ToRigid3d(tf_buffer.lookupTransform(
      //                      tracking_frame, "horizontal_vlp16_link", ToRos(point_t)));
      //       const carto::transform::Rigid3f sensor_to_map =
      //                   (tracking_to_map * sensor_to_tracking).cast<float>();
      //       human_points.push_back(sensor_to_map*p_xyz_local);
            
      //     }
      //     load_static_human_points.close();
      //     while(load_spin_human_points.getline(buffer,1024)){
      //       float values[4];
      //       std::stringstream ss;
      //       ss<<buffer;
      //       for(size_t i=0;i<4;++i){
      ...
      //       const carto::transform::Rigid3d sensor_to_tracking =
      //                   ToRigid3d(tf_buffer.lookupTransform(
      //                      tracking_frame, "vertical_vlp16_link", ToRos(point_t)));
      //       const carto::transform::Rigid3f sensor_to_map =
      //                   (tracking_to_map * sensor_to_tracking).cast<float>();
      //       human_points.push_back(sensor_to_map*p_xyz_local);
      //     }
      //   load_spin_human_points.close();
      //   std::cout<<"human_points size: "<<human_points.size()<<std::endl;
      // }

  std::vector<std::unique_ptr<carto::io::PointsProcessor>> pipeline =
      point_pipeline_builder_->CreatePipeline(
          lua_parameter_dictionary->GetDictionary("pipeline").get(),human_points);
