...
// added by galyean, to control the scale of trajectory in rviz view
constexpr double kTrajectoryLineStripMarkerScale = 0.3;
...
visualization_msgs::Marker CreateLandmarkMarkerText(int landmark_index,
                                                const std::string& index,
                                                const Rigid3d& landmark_pose,
                                                const std::string& frame_id) {
  // add by galyean
  visualization_msgs::Marker marker_text;
  marker_text.ns = "Landmarks";
  marker_text.id = landmark_index+1000;
  ...
  Rigid3d temp(landmark_pose.translation()+Eigen::Vector3d(.7,0,0.6),landmark_pose.rotation());
  marker_text.pose=ToGeometryMsgPose(temp);
  return marker_text;
}
...
visualization_msgs::Marker CreateLandmarkMarker(int landmark_index,
  ...
  marker.header.stamp = ::ros::Time::now();
  marker.header.frame_id = frame_id;
  marker.scale.x = 1.5*kLandmarkMarkerScale;
  marker.scale.y = 1.5*kLandmarkMarkerScale;
  marker.scale.z = 3*kLandmarkMarkerScale;
...
void MapBuilderBridge::LoadState(const std::string& state_filename,
                                 bool load_frozen_state,std::string lanmark_poses_file) {
  ...
  if(!lanmark_poses_file.compare("n")){
    map_builder_->LoadState(&stream, load_frozen_state);
  }else{
    map_builder_->LoadStateLandmark(&stream, lanmark_poses_file, load_frozen_state);
  }
...
void MapBuilderBridge::HandleLocalSubmapQuery(
    cartographer_ros_msgs::SubmapQuery::Response& response) {
  cartographer::mapping::proto::SubmapQuery::Response response_proto;
  const std::string error =
      map_builder_->LocalSubmapToProto(&response_proto);
  if (!error.empty()) {
    LOG(ERROR) << error;
...
void MapBuilderBridge::HandlePoseQuery(
  ...
  uint32_t timestamp = request.timestamp;// timestamp in request is ms
  double timestamp_s = timestamp/1e3;
  // if time==0, response latest pose
  if(nodes.size()==0){
    response.code = 0;
    return;
  }
  ::cartographer::common::Time pano_t = ::cartographer_ros::FromRos(ros::Time(timestamp_s));
  auto end = nodes.end();
  std::advance(end,-1);
  ros::Time time_ros = ::cartographer_ros::ToRos(end->data.time());
  std::cout<<"current pose_graph end time: "<<time_ros<<std::endl;
  if(std::abs(timestamp_s)<1e-5||pano_t > end->data.time()){
    std::cout<<"return latest pose\n";
    Rigid3d global_pose = end->data.global_pose;
    response.position[0] = global_pose.translation()[0];
    response.position[1] = global_pose.translation()[1];
    response.position[2] = global_pose.translation()[2];
    response.rotation[0] = global_pose.rotation().w();
    response.rotation[1] = global_pose.rotation().x();
    response.rotation[2] = global_pose.rotation().y();
    response.rotation[3] = global_pose.rotation().z();
    response.code = 1; 
  }else{
    
    bool miss_time = true;
    ::cartographer::transform::TimestampedTransform start_pose,end_pose;
    ::cartographer::transform::TimestampedTransform pre_pose;
    pre_pose.time = ::cartographer_ros::FromRos(ros::Time(timestamp_s+0.01f));
    for (const auto& node_id_data : nodes){
        if(node_id_data.data.time()>=pano_t&&pre_pose.time<=pano_t){
          start_pose = pre_pose;
          end_pose.time = node_id_data.data.time();
          end_pose.transform = node_id_data.data.global_pose;
          miss_time = false;
          break;
         // break
       }
       pre_pose.time = node_id_data.data.time();
       pre_pose.transform = node_id_data.data.global_pose;
    }
    if(miss_time){
      response.code = 0;
      return;
    }
    ros::Time time_start = ::cartographer_ros::ToRos(start_pose.time);
    ros::Time time_end = ::cartographer_ros::ToRos(end_pose.time);
    std::cout<<"Interpolate start time: "<<time_start<<std::endl;
    std::cout<<"Interpolate end time: "<<time_end<<std::endl;
    auto global_pose = Interpolate(start_pose,end_pose,pano_t);
    
    response.position[0] = global_pose.transform.translation()[0];
    response.position[1] = global_pose.transform.translation()[1];
    response.position[2] = global_pose.transform.translation()[2];
    response.rotation[0] = global_pose.transform.rotation().w();
    response.rotation[1] = global_pose.transform.rotation().x();
    response.rotation[2] = global_pose.transform.rotation().y();
    response.rotation[3] = global_pose.transform.rotation().z();
    response.code = 1;
  }
}
void MapBuilderBridge::HandleFenceSave(
      cartographer_ros_msgs::Fence::Request&request,
      cartographer_ros_msgs::Fence::Response&response){
        // add by galyean
        response.code =0;
        std::vector<Eigen::Vector3d> points_3d;
        uint16_t total_points = request.total_points;
        for(size_t i=0;i!=total_points;++i){
          points_3d.emplace_back(
            Eigen::Vector3d(request.points[i*3],request.points[i*3+1],0));
        }
        // get local pose 
        const auto nodes = map_builder_->pose_graph()->GetTrajectoryNodes();
        if(nodes.size()<1){
          std::cerr<<"zero optimized pose in the pose_graph, can not save fences\n";
          response.code = 0;
          return;
        }
        // 
        int pose_size = nodes.size();
        auto node_id = nodes.begin();
        std::advance(node_id,pose_size/2);
        Rigid3d global_pose = node_id->data.global_pose.inverse();
        	// save transform here
	      Eigen::Matrix4d matrix_G_to_B = Eigen::Matrix4d::Identity();
        matrix_G_to_B.block(0,0,3,3) = global_pose.rotation().toRotationMatrix();
        matrix_G_to_B.block(0,3,3,1) = global_pose.translation();
        
        
        ::cartographer::common::Time time = node_id->data.time();
	      ros::Time time_ros = ::cartographer_ros::ToRos(time);
        
        std::ofstream f_save(request.message+"/fence.txt");
        f_save<<std::fixed<<std::setprecision(9)<<time_ros<<std::endl;
        for(size_t i=0;i<points_3d.size();++i){
          f_save<<(matrix_G_to_B.block(0,0,3,3)*points_3d[i]
          +matrix_G_to_B.block(0,3,3,1)).transpose()<<std::endl;
        }
        f_save.close();

        std::ofstream f_save_trans(request.message+"/trans.txt");
	      f_save_trans<<std::fixed<<std::setprecision(9);
        f_save_trans<<matrix_G_to_B<<std::endl;
        f_save_trans<<time_ros<<std::endl;
        f_save_trans.close();
        response.code = 1;
      }
...
visualization_msgs::MarkerArray MapBuilderBridge::GetTrajectoryNodeList() {
  ...
  for (const int trajectory_id : node_poses.trajectory_ids()) {
    ...
    marker.color.a = 1.0;
    // auto end = node_poses.trajectory(trajectory_id).end();
    // std::advance(end,-1);
    // std::cout<<"local pose : "<<end->data.constant_pose_data.value().time<<"   "
    // <<end->data.constant_pose_data.value().local_pose<<"  \n  "
    // <<"    global pose :"<<end->data.global_pose<<std::endl;
...
visualization_msgs::MarkerArray MapBuilderBridge::GetLandmarkPosesList() {
  ...
  for (const auto& id_to_pose : landmark_poses) {
    ...
    landmark_poses_list.markers.push_back(CreateLandmarkMarkerText(
        GetLandmarkIndex(id_to_pose.first, &landmark_to_index_),
        id_to_pose.first,
        id_to_pose.second, node_options_.map_frame));
  }
