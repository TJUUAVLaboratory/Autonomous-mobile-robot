...
DEFINE_string(load_state_filename, "",
              ...
              "a saved SLAM state.");
DEFINE_string(landmark_poses_filename, "",
              "If non-empty, landmark poses file to load, containing "
              "landmark poses.");
...
void RunOfflineNode(const MapBuilderFactory& map_builder_factory) {
  ...
  if (!FLAGS_load_state_filename.empty()) {
    if(!FLAGS_landmark_poses_filename.empty()){
      node.LoadState(FLAGS_load_state_filename, FLAGS_load_frozen_state,FLAGS_landmark_poses_filename);
    }else{
