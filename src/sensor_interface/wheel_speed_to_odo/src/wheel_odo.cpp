...
public:
    ...
    Pose(double time,Eigen::Quaterniond &q,Eigen::Vector3d &trans):
    timestamp(time),q_i_g(q.cast<double>()),t_i_g(trans.cast<double>()){};
    double timestamp;
    Eigen::Quaterniond q_i_g;
    Eigen::Vector3d t_i_g;
};
struct WheelParameters{
    WheelParameters(float l_r,float r_r,float w_b):
    left_wheel_radius(l_r),right_wheel_radius(r_r),wheel_base(w_b){};
    float left_wheel_radius;// 150mm
    float right_wheel_radius;// 150mm
    float wheel_base;//412mm
};
struct WheelSpeed{
    WheelSpeed(double t,float r,float l):
            timestamp(t),right_rpm(r),left_rpm(l){};
    float left_rpm;
    float right_rpm;
    double timestamp;
};
...
void Node::HandleWheelSpeed(const robot_msgs::wheelSpeed::ConstPtr &msg) {
    ...
    int rpm = (int) speed;
   // std::cout<<(double)timestamp/1e9<<std::endl;
    std::cout<<"step 1\n";
    double time_stamp_sec = (double)timestamp_/1e4;
    //std::cout<<(double)timestamp_/1e9<<std::endl;
    std::cout<<"step 2\n";
    RPMData rpm_data;
