...
controller是局部规划
*********************************************************************/
#include "aibee_robot/aibee_navi.h"
#include <cmath>
...
MoveBase::MoveBase(tf::TransformListener &tf) : tf_(tf),
                                                actionServer_(NULL), //action server
                                                global_costmap_ros_(NULL),
                                                local_costmap_ros_(NULL),
                                                bgp_loader_("nav_core", "nav_core::BaseGlobalPlanner"), //global_planner plugin ClassLoader
                                                blp_loader_("nav_core", "nav_core::BaseLocalPlanner"),  //local_planner plugin ClassLoader
                                                recovery_loader_("nav_core", "nav_core::RecoveryBehavior"),

                                                //pose list
                                                global_planner_plan_(NULL), latest_plan_(NULL), local_planner_plan_(NULL),
                                                // flag
                                                runPlanner_(false),
                                                setup_(false),
                                                p_freq_change_(false), //global  planner frequence change
                                                c_freq_change_(false), //local   controler frequence change
                                                new_global_plan_(false)
{

  // action server  监听 move_base_msgs::MoveBaseGoal消息  回调函数 executeCb执行　global 进行执行局部路径规划循环
  actionServer_ = new MoveBaseActionServer(ros::NodeHandle(), "move_base", boost::bind(&MoveBase::executeCb, this, _1), false);
  ...
  local_planner_plan_ = new std::vector<geometry_msgs::PoseStamped>(); //control plan waypoints

  //set up the planner's thread  启动全局规划的线程
  ...
  // /move_base/goal 　target_pose send to MoveBaseActionServer action server
  ros::NodeHandle action_nh("move_base"); //发布 /move_base/goal 
  ...
  try
  {
    ...
    global_planner_->initialize(bgp_loader_.getName(global_planner), global_costmap_ros_); //初始化一个global_planner navfn_ros
    ROS_WARN("load global planner plugin: \r\n %s \r\n", global_planner.c_str());
  }
  ...
  local_costmap_ros_ = new costmap_2d::Costmap2DROS("local_costmap", tf_); //注意tf tree
  ...
  try
  {
    ...
    local_planner_->initialize(blp_loader_.getName(local_planner), &tf_, local_costmap_ros_);
    ROS_WARN("load local planner plugin: \r\n %s \r\n", local_planner.c_str());
  }
  catch (const pluginlib::PluginlibException &ex)
  {
  ...
  if (shutdown_costmaps_)
  {
    ...
    global_costmap_ros_->stop();
    local_costmap_ros_->stop();
  }
  ...
  actionServer_->start();
  ROS_WARN("MoveBaseAction service started, ready to get a goal and make plan.....");
...
void MoveBase::reconfigureCB(move_base::MoveBaseConfig &config, uint32_t level)
{
  ...
  if (config.base_global_planner != last_config_.base_global_planner)
  {
    ...
    try
    {
      ...
      latest_plan_->clear();
      local_planner_plan_->clear();
      resetState();
      global_planner_->initialize(bgp_loader_.getName(config.base_global_planner), global_costmap_ros_);
  ...
  if (config.base_local_planner != last_config_.base_local_planner)
  {
    ...
    try
    {
      ...
      local_planner_plan_->clear();
      resetState();
      local_planner_->initialize(blp_loader_.getName(config.base_local_planner), &tf_, local_costmap_ros_);
    }
    catch (const pluginlib::PluginlibException &ex)
    {
      ROS_FATAL("Failed to create the %s planner, are you sure it is properly registered and that the \
...
void MoveBase::clearCostmapWindows(double size_x, double size_y)
{
  ...
  global_costmap_ros_->getRobotPose(global_pose);
  ...
  global_costmap_ros_->getCostmap()->setConvexPolygonCost(clear_poly, costmap_2d::FREE_SPACE);

  //clear the controller's costmap
  local_costmap_ros_->getRobotPose(global_pose);
  ...
  pt.y = y + size_y / 2;
  clear_poly.push_back(pt);
  local_costmap_ros_->getCostmap()->setConvexPolygonCost(clear_poly, costmap_2d::FREE_SPACE);
}
...
bool MoveBase::clearCostmapsService(std_srvs::Empty::Request &req, std_srvs::Empty::Response &resp)
{
  ...
  //clear the costmaps
  boost::unique_lock<costmap_2d::Costmap2D::mutex_t> lock_controller(*(local_costmap_ros_->getCostmap()->getMutex()));
  local_costmap_ros_->resetLayers();

  boost::unique_lock<costmap_2d::Costmap2D::mutex_t> lock_planner(*(global_costmap_ros_->getCostmap()->getMutex()));
...
bool MoveBase::planService(nav_msgs::GetPlan::Request &req, nav_msgs::GetPlan::Response &resp)
{
  ...
  {
    tf::Stamped<tf::Pose> global_pose;
    if (!global_costmap_ros_->getRobotPose(global_pose))
    {
  ...
  if (!global_planner_->makePlan(start, req.goal, global_plan) || global_plan.empty())
  {
    ...
    float resolution = global_costmap_ros_->getCostmap()->getResolution();
    float search_increment = resolution * 3.0; //偏移的步长
    if (req.tolerance > 0.0 && req.tolerance < search_increment) // such search_increment=0.5  search_increment=0.05*3
      search_increment = req.tolerance; //限幅
...
MoveBase::~MoveBase()
{
  ...
  if (actionServer_ != NULL)
    delete actionServer_;

  if (global_costmap_ros_ != NULL)
    delete global_costmap_ros_;

  if (local_costmap_ros_ != NULL)
    delete local_costmap_ros_;
...
bool MoveBase::makePlan(const geometry_msgs::PoseStamped &goal, std::vector<geometry_msgs::PoseStamped> &plan)
{
  ...
  // lock costMap
  boost::unique_lock<costmap_2d::Costmap2D::mutex_t> lock(*(global_costmap_ros_->getCostmap()->getMutex()));
...
geometry_msgs::PoseStamped MoveBase::goalToGlobalFrame(const geometry_msgs::PoseStamped &goal_pose_msg)
{
  ...
  std::string global_frame = global_costmap_ros_->getGlobalFrameID(); //map
...
void MoveBase::planThread()
{
  ...
  ROS_WARN("global_planner planThread");
  ros::NodeHandle n;
  ros::Timer timer;
  ...
  while (n.ok())
  {
    ...
    while (wait_for_wake || !runPlanner_) // 等待runPlanner标志位
    {
      //if we should not be running the planner then suspend this thread
      ROS_DEBUG_NAMED("plan_thread", "Planner thread is suspending");
    ...
    lock.unlock();
    ROS_DEBUG_NAMED("plan_thread", "Planning...");

    //run planner 全局路径规划
    global_planner_plan_->clear(); //planner plan vector清空
    bool gotPlan = n.ok() && makePlan(temp_goal, *global_planner_plan_);

    if (gotPlan)
    {
      ROS_DEBUG_NAMED("move_base_plan_thread", "Got Plan with %zu points!", global_planner_plan_->size());
      //pointer swap the plans under mutex (the controller will pull from latest_plan_)
      std::vector<geometry_msgs::PoseStamped> *temp_plan = global_planner_plan_;
    ...
    {
      ROS_DEBUG_NAMED("plan_thread", "No Plan...  cann't get goal  or  the goal in the obstacle area!");
      ros::Time attempt_end = last_valid_plan_ + ros::Duration(planner_patience_); //5秒钟
...
void MoveBase::executeCb(const move_base_msgs::MoveBaseGoalConstPtr &move_base_goal)
{
  ...
  {
    //
    actionServer_->setAborted(move_base_msgs::MoveBaseResult(), "Aborting on goal because it was sent with an invalid quaternion");
    return;
  }
  ...
  while (n.ok())
  {
    ...
    if (actionServer_->isPreemptRequested())
    {
      if (actionServer_->isNewGoalAvailable())
      {
        ...
        move_base_msgs::MoveBaseGoal new_goal = *actionServer_->acceptNewGoal();
      ...
      { // actionServer_->isNewGoalAvailable() false
    ...
    if (goal.header.frame_id != global_costmap_ros_->getGlobalFrameID())
    {
  ...
  actionServer_->setAborted(move_base_msgs::MoveBaseResult(), "Aborting on the goal because the node has been killed");
  return;
}
...
/*
  ...
  2. publishFeedback
  3. check oscillation
  4. check local_costmap_ros_->isCurrent()
...
bool MoveBase::executeCycle(geometry_msgs::PoseStamped &goal, std::vector<geometry_msgs::PoseStamped> &global_plan)
{
  ...
  tf::Stamped<tf::Pose> global_pose;
  global_costmap_ros_->getRobotPose(global_pose); // tf中得到位置
  ...
  actionServer_->publishFeedback(feedback); // moveAction server反馈当前机器人的位姿
  ...
  if (!local_costmap_ros_->isCurrent())
  {
  ...
  { //makePlan got global plan path
    ...
    std::vector<geometry_msgs::PoseStamped> *temp_plan = local_planner_plan_;

    boost::unique_lock<boost::recursive_mutex> lock(planner_mutex_);
    local_planner_plan_ = latest_plan_; // global_plan list
    latest_plan_ = temp_plan;
    lock.unlock();
    ROS_DEBUG_NAMED("move_base", "pointers swapped!");

    //全局路径规划 --> 局部路径规划
    if (!local_planner_->setPlan(*local_planner_plan_))
    {
      ...
      actionServer_->setAborted(move_base_msgs::MoveBaseResult(), "Failed to pass global plan to the controller.");
      return true;
    }
  ...
  case CONTROLLING: //局部规划
    ...
    if (local_planner_->isGoalReached())
    {
      ...
      actionServer_->setSucceeded(move_base_msgs::MoveBaseResult(), "Goal reached.");
      return true;
    }
    ...
    //局部路径规划
    {
      boost::unique_lock<costmap_2d::Costmap2D::mutex_t> lock(*(local_costmap_ros_->getCostmap()->getMutex()));
  ...
  case CLEARING:
    ...
    else
    {
      ...
      if (recovery_trigger_ == CONTROLLING_R)
      {
        ...
        actionServer_->setAborted(move_base_msgs::MoveBaseResult(), "Failed to find a valid control. Even after executing recovery behaviors.");
      }
      else if (recovery_trigger_ == PLANNING_R)
      {
        ROS_ERROR("Aborting because a valid plan could not be found. Even after executing all recovery behaviors");
        actionServer_->setAborted(move_base_msgs::MoveBaseResult(), "Failed to find a valid plan. Even after executing recovery behaviors.");
      }
      else if (recovery_trigger_ == OSCILLATION_R)
      {
        ROS_ERROR("Aborting because the robot appears to be oscillating over and over. Even after executing all recovery behaviors");
        actionServer_->setAborted(move_base_msgs::MoveBaseResult(), "Robot is oscillating. Even after executing recovery behaviors.");
      }
      resetState();
      return true;
    }
    ...
    if (behavior_list.getType() == XmlRpc::XmlRpcValue::TypeArray)
    {
      ...
      for (int i = 0; i < behavior_list.size(); ++i)
      {
        try
        {
          ...
          behavior->initialize(behavior_list[i]["name"], &tf_, global_costmap_ros_, local_costmap_ros_);
...
void MoveBase::loadDefaultRecoveryBehaviors()
{
  ...
  try
  {
    ...
    cons_clear->initialize("conservative_reset", &tf_, global_costmap_ros_, local_costmap_ros_);
    ...
    {
      rotate->initialize("rotate_recovery", &tf_, global_costmap_ros_, local_costmap_ros_);
      recovery_behaviors_.push_back(rotate);
    }

    //next, we'll load a recovery behavior that will do an aggressive reset of the costmap
    boost::shared_ptr<nav_core::RecoveryBehavior> ags_clear(recovery_loader_.createInstance("clear_costmap_recovery/ClearCostmapRecovery"));
    ags_clear->initialize("aggressive_reset", &tf_, global_costmap_ros_, local_costmap_ros_);
